...
def apply_discount(price: float, discount_tier: str) -> float:
    ...
    return round(price * (1.00 - discount / 100.00), 2)
...
def ollama_chat_traced(message):
    return ollama.chat(model=MODEL, tools=tools_for_llm, messages=message)
...
def run_agent(question: str):
    ...
    for iteration in range(1, MAX_ITERATIONS + 1):
        ...
        response = ollama_chat_traced(message=messages)
